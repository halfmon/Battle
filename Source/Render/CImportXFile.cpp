...
#define INITGUID // Ensure GUIDs get defined
#include <windows.h>
#include <dxfile.h>
#include <rmxfguid.h>
#include <rmxftmpl.h>
...
) const
{
	...
	if (m_Materials[iMaterial].sTextureName == "")
	{
		...
		return VertexLitNoTex;
	}
	else
	{
		if (pNumTextures) *pNumTextures = 1;
		if (m_Materials[iMaterial].sTextureName == "BRANCH_W.tga" ||
			m_Materials[iMaterial].sTextureName == "BRUHCH_G.tga")
		{
			return VertexLitAlphaTest;
		}
		else if (m_Materials[iMaterial].sName.find( "PlainTexture" ) != string::npos )
		{
			return PlainTexture;
		}
		else
