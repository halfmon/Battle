...
public:
	...
	// Create a character entity template with the given type, name, mesh and stats. Returns the new
	// charater template pointer
	CCharTemplate* CEntityManager::CreateCharTemplate( const string& type, const string& name,
	                                                   const string& mesh, TInt32 maxHealth,
													   TInt32 maxMagic, TInt32 st, TInt32 in, 
													   TInt32 sp, vector<CAttack> attacks, vector<CDefence> defences,
	...
	// Create a character, requires a character template name, may supply entity name and position
	// Returns the UID of the new entity
	TEntityUID CreateCharacter
	(
		const string&   templateName,
		const string&   name = "",
		const CVector3& position = CVector3::kOrigin, 
		const CVector3& rotation = CVector3( 0.0f, 0.0f, 0.0f ),
		const CVector3& scale = CVector3( 1.0f, 1.0f, 1.0f ),
		const TFloat32& strength = 0.5f,
		const TFloat32& intelligence = 0.5f,
		const TFloat32& speed = 0.5f
	);
	...
	CCharEntity* GetCharEntity( TEntityUID UID )
	{
		TUInt32 entityIndex;
		if (!m_EntityUIDMap->LookUpKey( UID, &entityIndex ))
		{
		...
		return static_cast<CCharEntity*>(m_Entities[entityIndex]);
	}
