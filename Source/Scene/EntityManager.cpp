...
TEntityUID CEntityManager::CreateCharacter
(
	const string&   templateName,
	const string&   name,
	...
	const TFloat32& strength,
	const TFloat32& intelligence,
	const TFloat32& speed
)
{
	CCharTemplate* charTemplate = static_cast<CCharTemplate*>(GetTemplate( templateName ));

	CEntity* newEntity = new CCharEntity( charTemplate, m_NextUID, name, position, rotation, scale );
